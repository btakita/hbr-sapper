...
function extract_css(client_result, components, dirs) {
 ...
 = {
	...
	client_result.chunks.forEach(chunk=>{
		...
		writeFileAtomic.sync(`${asset_dir}/${output_file_name}`, `${code}\n/* sourceMappingURL=./${output_file_name}.map */`);
		writeFileAtomic.sync(`${asset_dir}/${output_file_name}.map`, JSON.stringify(map, null, '  '));
	...
	fs.readdirSync(asset_dir).forEach(file=>{
		...
		writeFileAtomic.sync(`${asset_dir}/${file}`, replaced);
	});
